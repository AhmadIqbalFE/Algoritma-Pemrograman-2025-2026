print('---Belajar Tipe Data---') 
 
# Tipe data integers x = 5 
y = 3 
z = 5 + 3 
print(z) 
 
# Tipe data floating numbers 
a = 7.5 
print(a) 
 
# Menggunakan single quote 
myFirstName = 'Ahmad' 
 
# Menggunakan double quote 
myLastName = "Iqbal" 
 
# Penulisan Angka sebagai string 
myAge = "36" 
 
# Penulisan string multiline 
myHobby = """Hobi saya adalah Travelling. """ 
 
# Mencetak 2 variabel string dengan perintah print print(myFirstName + " " + myLastName) print("Umur saya " + myAge) 
print("Saya Ahmad Iqbal " + myHobby) 
 
# Contoh penulisan boolean yang benar 
a = True 
b = False 
 
# Contoh penulisan boolean yang salah # Jika dijalankan, pada baris ini akan error 
c = a 
 
print(c) 
